...
- [ ] Per-app capture history: an `app_capture_sessions` table, one row per enable and per disable of a source app (INB-10, INB-22, CAP-12)
- [ ] Outbound send state: `messages.send_state` in (`pending`, `sent`, `failed`) beside `direction` (INB-9)
- [ ] The shipped messaging-app list as a single artifact, `lib/data/shipped_apps.dart`, the one source CAP-1's filter, INB-21's chooser, PERM-3's disclosure and the manifest's `<queries>` all read (INB-20, INB-16, PERM-3, CAP-1)
- [ ] The listener's `apps` row for a package it is not capturing from: package, label, `last_seen_at`, `enabled` false, written before the notification is dropped (INB-20, CAP-1, CAP-15, CAP-27)
